%----------------------------------------------------
% Usado en figura poster, rata deprivada
%----------------------------------------------------
...
hold all;
plot([130; 130], [-1; -.5], '-k',  [130; 140], [-1; -1], '-k', 'LineWidth', 2)
%plot(bar1(:,1),bar1(:,2),'k','linewidth',8); % MUSC application
% ---- scale bar ----
axis off;

xlim([0 150]); % 30 seconds
ylim([-1 1]);
box off;
...
subplot(6,4,21);
m1=mean(spikes(s.cinco(10:60),:));
m2=mean(spikes(s.cinco(end-20:end),:));
er1=std(spikes(s.cinco(1:60),:))/sqrt(length(spikes(s.cinco(1:60))));
