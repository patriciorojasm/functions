...
names =who('spike*');
 ...
 for i=1:size(names,1)          
     ...
     if     t_rate_max{i}(j,1)   >0 &&  t_rate_max{i}(j,1)  <11 %temp entre 0 y 5 grad
         %subplot(511);
         figure(f2); subplot(211);
         plot( rate_m{i}(:,1), rate_m{i}(:,j), 'b-o'); 
         plot(ah3, rate_m{i}(:,1), rate_m{i}(:,j), 'b-o','linewidth', 2,'markersize',8); 
     elseif t_rate_max{i}(j,1)   >11.0001 &&  t_rate_max{i}(j,1)  <25 %temp entre 0 y 5 grad
         figure(f2);subplot(212);
         plot( rate_m{i}(:,1), rate_m{i}(:,j), 'r-o'); 
         plot(ah3, rate_m{i}(:,1), rate_m{i}(:,j), 'r-o','linewidth', 2,'markersize',8); 
 ...
 figure(f3); box off;set(gca,'TickDir','out');set(gca, 'FontName', 'arial');
 set(gca, 'fontsize', 10, 'linewidth', 2,'FontWeight', 'bold');ylabel('Firing Rate (Hz)');xlabel('Temp [C]'); 
 
 print('fig_inf_final', '-r150','-depsc2')
 disp(count)
 disp(count1)
