%------------------------------------
% for use with concat_traces_time_window.m
%------------------------------------
...
figure ('OuterPosition',[2200 200 400 800]); hold all; 
subplot(212);
%data=data';
plot(time_new, data);
...
f_1=figure('OuterPosition',[50 200 1800 600]);  
plot(time_new, data);
c=0;
hold all;
c_num =max(cluster_class(:,1));
...
for c=0:c_num+1;
    ...
    plot( [t(j)+time_window(1);t(j)+time_window(1)],[40*ones(size(t(j)));zeros(size(t(j)))], 'k', 'color',cc(c+1,:));    %'color',rand(1,3));
...
i=1;
c=0;
f_2 =figure('OuterPosition',[1800 400 400 400]);  hold all;
hold all;
c_num =max(cluster_class(:,1));
%subplot(c_num,1,c); 
for c=0:c_num
...
f_3 = figure('OuterPosition',[2200 500 700 800]);  
for k=0:c_num
    %----------------------
    % define j as GLOBAL
    %----------------------
