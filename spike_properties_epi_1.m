...
for c=0:c_num
    ...
    %p_uno= spikes(j,20);
    p.(names{c+1}) = spikes(s.(names{c+1}),10); % peak amplitude
    %plot(tim.(names{c+1}), p.(names{c+1}),'oc','color',cc(c+1,:));  % ALT
    plot(p.(names{c+1}),'oc','color',cc(c+1,:));
    %subplot(c_num+1,2,c+2);
    %hist(p.(names{c+1}),1000,'color',cc(c+1,:));%hold all;
end
...
for c=0:c_num
    ...
    %p_uno= spikes(j,20);
    p.(names{c+1}) = spikes(s.(names{c+1}),10); % peak amplitude
    %------removed 31.12.2015 because it was not working---------
    %plot(tim.(names{c+1}), p.(names{c+1}),'oc','color',cc(c+1,:));     
    %------added 31.12.2015 to improve line 21---------
...
%{
f_7_c=figure('OuterPosition',[1300 500 500 800]);
for c=0:c_num
    p.(names{c+1}) = spikes(s.(names{c+1}),25); % peak amplitude
    j=find(cluster_class(:,1)==c);  % indice
    t=cluster_class(:,2);           % column with times
    t=.001*t';
    plot(t(j), p.(names{c+1}),'oc','color',cc(c+1,:));    hold on;
    xlim([time_new(1) time_new(end)]);
end
%}
%{
